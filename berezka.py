...
class ClssDialog(QtWidgets.QDialog):
    def __init__(self, chosen_test):
        self.chosen_test = chosen_test
        if self.chosen_test == 't_60':
        ...
        if len(self.users_answers) == 60:
            ...
            for i in self.users_answers:
                ...
                #баг
                    ...
                    for i in range(2, 60):
                        print(self.right_answers[self.count])
...
class MyWidget(QMainWindow):
    def __init__(self, t_60=None,):
        super().__init__()
        self.runner = False
        uic.loadUi('design.ui', self)  # Загружаем дизайн
        self.test_60.clicked.connect(self.run_test('t_60'))
        #self.test_orthoepy.clicked.connect(self.run_test_orthoepy)

    def run_test(self, chosen_test):
        #ex.close()
        self.runner = True
        test = ClssDialog(chosen_test)
        test.exec_()
